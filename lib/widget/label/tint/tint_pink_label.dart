...
class TintPinkLabel extends StatelessWidget {
  final String text;
  final String? icon;
          ...
          children: [
            if (icon != null)
              SvgPicture.asset(
                ...
                width: 16,
                height: 16,
                colorFilter: const ColorFilter.mode(ColorPalette.pink, BlendMode.srcIn),
              ),
            if (icon != null) const SizedBox(width: 2), // 이모지와 텍스트 사이의 간격
            Text(
              text,
              style: TextStyles.subTitleXxSmall(color:ColorPalette.pink),
            ),
          ],
        ),
      ),
